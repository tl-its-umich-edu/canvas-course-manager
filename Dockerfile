...
FROM node:12-alpine AS node-webpack1
WORKDIR /usr/src/app
...
FROM node:12-alpine AS node-prod-deps1

COPY --from=node-webpack1 /usr/src/app /usr/src/app
...
RUN apt-get update && \
    apt-get install -y --no-install-recommends netcat vim-tiny jq python3-dev xmlsec1 && \
    apt-get upgrade -y && \
    apt-get clean -y && \
    pip install -r requirements.txt

# copy built react and node libraries for production mode
COPY --from=node-prod-deps1 /usr/src/app/package-lock.json package-lock.json
COPY --from=node-prod-deps1 /usr/src/app/webpack-stats.json webpack-stats.json
COPY --from=node-prod-deps1 /usr/src/app/assets assets
COPY --from=node-prod-deps1 /usr/src/app/node_modules node_modules
